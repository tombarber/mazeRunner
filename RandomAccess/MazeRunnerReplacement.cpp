...
const char levels[] PROGMEM = {
 ...
 int straight_max_speed = 60*speed_modifier;
 int turning_max_speed = 80*speed_modifier;
 int delay_ms = 175*speed_modifier;
...
unsigned int MazeRunner::straightForDistance(unsigned int distanceUnits) {
  int last_proportional = 0;
  long integral=0;
  unsigned long startTime = millis();
  while(1) {
    ...
    // sensors 0 and 4 for detecting lines going to the left and
    // right.
    if(round((millis() - startTime) / unit_time) >= distanceUnits -1) {
		return;
	}
  }
}
...
void MazeRunner::directionsAvailableNew(unsigned int *direction_array) {
  read_line(sensors,IR_EMITTERS_ON);
...
void MazeRunner::roundedTurn(char dir) {
  switch(dir) {
    case 'L':
    case 'l':
      setSpeedsFor(0, 1 * turning_max_speed, 2 * delay_ms);
      break;
    case 'R':
    case 'r':
      // Turn right.
      setSpeedsFor(1 * turning_max_speed, 0, 2 * delay_ms);
      break;
    case 'B':
    case 'b':
      // Turn around.
